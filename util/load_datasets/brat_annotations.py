...
class Annotation_Habernal:
    def __init__(self, id, label, start, end, file, text=""):
        ...
        # assign label
        if label == "background_claim":
            self.label = Label_Habernal.BACKGROUND_CLAIM
        elif label == "own_claim":
            self.label = Label_Habernal.OWN_CLAIM
        elif label == "data":
            self.label = Label_Habernal.DATA
        elif label == "supports":
            self.label = Label_Habernal.SUPPORTS
        elif label == "contradicts":
            self.label = Label_Habernal.CONTRADICTS
        elif label == "parts_of_same":
            self.label = Label_Habernal.PARTS_OF_SAME
        elif label == "semantically_same":
            self.label = Label_Habernal.SEMANTICALLY_SAME
                ...
                with codecs.open(os.path.join(subdir, file), mode="r", encoding="utf8") as ann_file:
                    for line in ann_file:
                        try:
                            ...
                            annotations.append(
                                Annotation_Lauscher(id=id, label=label, start=start, end=end, file=file, text=text))
...
# annotations = parse_annotations(r"C:\Users\Wifo\PycharmProjects\Masterthesis\data\Argument_Component_Identification_Habernal\brat-project-final")
...
print(type(annotations_Lauscher))
ACI_Lauscher = pd.DataFrame(annotations_Lauscher)
print(ACI_Lauscher.head())
