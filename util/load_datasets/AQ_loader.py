...
def load_QualityPrediction_datset_2part(test_set):
    # print("Loading Argument Quality Prediction Dataset")
    if test_set:
        path = "./data/Argument_Quality/9.1_test"
        all_files = glob.glob(path + "/*.csv")
    else:
        path = "./data/Argument_Quality/9.1_train_dev"
        all_files = glob.glob(path + "/*.csv")
...
def load_ArgQuality_datset(case_ID=4):
    ...
    """
    pickled = True
    container_file = get_path_to_OS() + "/util/pkl/AQ_data.pkl"
    path = get_path_to_OS() + "/data/Argument_Quality/IBM-ArgQ-9.1kPairs"
