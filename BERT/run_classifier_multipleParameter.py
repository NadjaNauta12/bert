...
from util.load_datasets import ACI_loader_Stab, AQ_loader, AZ_loader, ISA_loader, AR_loader, load_conll, \
...
class InputExample_Sequence(object):
    ...
    def __init__(self, guid, token_list, label_list=[]):
        """Constructs a InputExample.
        Args:
          guid: Unique id for the example.
          token_list: list of string. Contains tokens that belong to one sentence.
          label_list: list of strins. Contais the labels for each token that is given in the token list.
        """
        self.guid = guid
    ...
    def __repr__(self):
        return "{}({!r})".format(self.guid, " ".join(self.token_list[:10]), " ".join(self.label_list[:10]))
...
class InsufficientSupportProcessor(DataProcessor):
    ...
    def read_data_splitting(self, idx):
        path = get_path_to_OS() + '/data/Insufficient_Arg_Support/data-splitting.tsv'
...
class ArgRecognitionProcessor(DataProcessor):
    ...
    def get_experimental_setting():
        if ArgRecognitionProcessor._AR_flag_GM:
            ...
            return "UGIP"
        elif ArgRecognitionProcessor._AR_flag_GM_UGIP:
            return "GM_UGIP"
    ...
    def set_experimental_setting(setting):
        if setting == "GM":
            ...
            ArgRecognitionProcessor._AR_flag_GM = False
        if setting == "UGIP":
            ArgRecognitionProcessor._AR_flag_UGIP = True
        else:
            ArgRecognitionProcessor._AR_flag_UGIP = False
        if setting == "GM_UGIP":
            ArgRecognitionProcessor._AR_flag_GM_UGIP = True
        else:
            ArgRecognitionProcessor._AR_flag_GM_UGIP = False
        if setting == "UGIP_GM":
            ArgRecognitionProcessor._AR_flag_UGIP_GM = True
        else:
            ArgRecognitionProcessor._AR_flag_UGIP_GM = False
...
class ACI_Lauscher_Processor(DataProcessor):
    ...
    def _flat_sentences_to_InputExamples(self, flat_sentences, descr):
        ...
        for ele in flat_sentences:
            ...
            examples_list.append(InputExample_Sequence(guid=guid, token_list=words, label_list=labels))
            counter += 1
    ...
    def get_labels(self):
        ...
        return ['Token_Label.OUTSIDE', 'Token_Label.BEGIN_BACKGROUND_CLAIM', 'Token_Label.INSIDE_BACKGROUND_CLAIM',
                'Token_Label.BEGIN_DATA', 'Token_Label.INSIDE_DATA', 'Token_Label.BEGIN_OWN_CLAIM',
                'Token_Label.INSIDE_OWN_CLAIM', '[CLS]', '[SEP]']


class ACI_Stab_Processor(DataProcessor):

    def _get_examples(self, data_dir, descr):
        if descr == "Train":
            c = load_comp.load_ACI_Stab(caseID=1)
        elif descr == "Dev":
            c = load_comp.load_ACI_Stab(caseID=2)
        elif descr == "Test":
            c = load_comp.load_ACI_Stab(caseID=3)
        else:
            return None
    ...
    def get_labels(self):
        """Gets the list of labels for this data set."""
        # return ['Label_Stab.MAJOR_CLAIM', 'Label_Stab.CLAIM', 'Label_Stab.PREMISE',
        #         'Label_Stab.SUPPORTS', 'Label_Stab.ATTACKS']
...
def map_labels_to_tokens(words, token_list, label_list, tokenizer):
    label_sequence = []
    idx_word = 0
    idx_token = 0
    assert len(words) == len(label_list)
    while idx_word < len(words) and idx_token < len(token_list):
        if words[idx_word].lower() == token_list[idx_token]:
            label_sequence.append(label_list[idx_word])
            idx_token += 1
            idx_word += 1
        else:
            idx_range = len(tokenizer.tokenize(words[idx_word]))
            delta = 0
            ...
            if idx_range > 0:
                ...
                idx_word +=1
    assert len(label_sequence) == len(token_list)
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    if "ACI" in FLAGS.task_name:
        return _convert_single_example_ACI(ex_index, example, label_list, max_seq_length,
                           tokenizer)

    if isinstance(example, PaddingInputExample) and "ACI" not in FLAGS.task_name:
    ...
    if isinstance(example, PaddingInputExample) and "ACI"  in FLAGS.task_name:
        return InputFeatures(
            input_ids=[0] * max_seq_length,
            input_mask=[0] * max_seq_length,
            segment_ids=[0] * max_seq_length,
...
def _convert_single_example_ACI(ex_index, example, label_list, max_seq_length,
                           tokenizer):
    """Converts a single `InputExample` into a single `InputFeatures`."""
    ...
    assert len(example.label_list) == len(example.token_list)
    original_seq_length = len(example.token_list)
    # tokenize sentence
    tokenized_sentence = tokenizer.tokenize(" ".join(example.token_list))

    # required to add labels bc of tokenization?
    if len(tokenized_sentence) == original_seq_length:
        label_sequence = example.label_list
    else:  # longer (main) or shorter seq. possible (outlierts
        label_sequence = map_labels_to_tokens(example.token_list, tokenized_sentence, example.label_list, tokenizer)

    assert len(label_sequence) == len(tokenized_sentence)

    # just to make sure the cut is made here (again)
    # cut sentence which are too long
    if len(tokenized_sentence) > max_seq_length - 2:
        tokenized_sentence = tokenized_sentence[0:(max_seq_length - 2)]

    if len(label_sequence) > max_seq_length - 2:
        label_sequence = label_sequence[0:(max_seq_length - 2)]

    # convention of BERT - Data to BERT input
    tokens = []
    segment_ids = []
    label_ids = []
    ...
    for i, token in enumerate(tokenized_sentence):
        tokens.append(token)
        segment_ids.append(0)
        label_ids.append(label_map[label_sequence[i]])
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # switch between ACI Input Example representation and other standard one
        if "ACI" in FLAGS.task_name:
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    if "ACI" in FLAGS.task_name:
        name_to_features = {
            ...
            "label_ids": tf.FixedLenFeature([seq_length], tf.int64), # changed this
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    if "ACI" in FLAGS.task_name:
         (loss, per_example_loss, logits, probabilities) = _create_model_ACI(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings)
    ...
    final_hidden_shape = modeling.get_shape_list(output_layer, expected_rank=2)
    ...
    with tf.variable_scope("finetuning_output"): # changed this for finetuning
        if is_training:
            # I.e., 0.1 dropout
        ...
        probabilities = tf.nn.softmax(logits, axis=-1) # axis -1 sinde last dim of tensor contains labels -> 32 probs a 5 labels
...
def _create_model_ACI(bert_config, is_training, input_ids,
    ...
    final_hidden_shape = modeling.get_shape_list(
        output_layer, expected_rank=3)  # embedding size
    batch_size = final_hidden_shape[0]
    seq_length = final_hidden_shape[1]
    hidden_size = final_hidden_shape[2]

    used = tf.sign(tf.abs(input_ids))  # why not use the input_mask?
    lengths = tf.reduce_sum(used, reduction_indices=1)  # (batch_size) 32xSumme - Summe = Anzahl real elems

    output_weights = tf.get_variable( "output_weights",
        [num_labels, hidden_size],  initializer=
    ...
    with tf.variable_scope("finetuning_output"):  # changed this for finetuning
        ...
        reshape_output = tf.reshape(output_layer, [-1,
                                                   hidden_size])
        # shape [batch_size*seq_length, hidden]
        output_layer = reshape_output

        # unnormalized output of last layer
        logits = tf.matmul(output_layer, output_weights, transpose_b=True)
        # shape [batch_size*seq_length, num_labels]
        logits = tf.nn.bias_add(logits, output_bias)

        # shape [batch_size*seq_length, num_labels]
        probabilities = tf.nn.softmax(logits, axis=-1)  # shape [batch_size*seq_length, num_labels]
        probabilities = tf.reshape(probabilities, [-1, seq_length, num_labels])  # reshape for output

        one_hot_labels = tf.one_hot(labels, depth=num_labels, dtype=tf.float32)
        # 1-hot musst have same size as logits
        weights_sequence = tf.cast(input_mask, dtype=tf.float32)
        logits = tf.reshape(logits, [-1, seq_length, num_labels])  # (batch_size, seq_length, num_labels)

        loss = tf.losses.softmax_cross_entropy(
            onehot_labels=one_hot_labels,
            logits=logits,
            weights=weights_sequence,
        )

        do_not_reduce = tf.losses.Reduction.NONE
        per_example_loss = tf.losses.softmax_cross_entropy(
            onehot_labels=one_hot_labels,
            logits=logits,
            weights=weights_sequence,
            reduction=do_not_reduce
        )
            ...
            d = tf.data.Dataset.from_tensor_slices({
                ...
                "segment_ids":
                    tf.constant(
                        ...
                        dtype=tf.int32),
                "label_ids":
                    tf.constant(all_label_ids, shape=[num_examples, seq_length], dtype=tf.int32), # changed this
            })
        else:
            d = tf.data.Dataset.from_tensor_slices({
                "input_ids":
                ...
                "segment_ids":
                    tf.constant(
                        ...
                        dtype=tf.int32),
                "label_ids":
                    tf.constant(all_label_ids, shape=[num_examples], dtype=tf.int32),       # this is different
            })
...
def main_run_classifier(_, config_str, train_batch_size, learning_rate, num_train_epochs):
    ...
    abc = tokenizer.tokenize("We are mainly introduced to products through advertisements . Therefore, advertisers push the limits of creativity to dispose the consumers to purchase the product. When the consumers are impressed by the way a product is advertised , they can be convinced to consider that the product is a need in some cases . Recently , there is a very creative advertisements of a soft drink product on TV . The story delivers a desired call to drink that soft drink that people tend to drink when the weather is too hot . As a result of this , the number of that product being sold will increases .")
    ...
    if FLAGS.do_predict:
        ...
        if "ACI" in FLAGS.task_name:
            write_original_labels_ACI(predict_examples, config_str, FLAGS.task_name, output_dir)
        else:
            ...
            for (i, prediction) in enumerate(result):
                ...
                if "aci" in task_name:
                    ...
                    for token in probabilities:
                        output_line = "\t".join(str(label_prob) for label_prob in token) + "\n"
                        sentence = sentence + output_line
...
def write_original_labels_ACI(examples, config, task, path):
    print("Writing original Labels")
    output_path = path + "/" + task + "-goldlabels.tsv"
    with codecs.open(output_path, "w", "utf8") as f_out:
        f_out.write("index\ttoken\tlabel\n")
        for i, ele in enumerate(examples):
            tokens = ele.token_list
            labels = ele.label_list
            assert len(tokens) == len(labels)
            for idx in range(0, len(tokens)):
                f_out.write(str(i) + "\t" + tokens[idx] + "\t" + labels[idx] + "\n")
                # f_out.write(str(i) + "\t" + tokens[idx].encode('utf-8').strip() + "\t" + labels[idx].encode(
                #    'utf-8').strip() + "\n")

            f_out.write("\n")
        f_out.close()

def set_experimental_setting_ARTask(settingtxt="GM"):
    setting = settingtxt #  "GM_UGIP"  # "UGIP" #"GM" # "UGIP", "GM_UGIP", "UGIP_GM"
