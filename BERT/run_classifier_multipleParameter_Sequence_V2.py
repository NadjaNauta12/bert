...
class InputFeatures(object):
    ...
    # TODO No adjustments according to NER example
    def __init__(self,
                 input_ids,
                 input_mask,
                 segment_ids,
...
class InsufficientSupportProcessor(DataProcessor):
    ...
    def read_data_splitting(self, idx):
        ...
        path = ""  # TODO PATH  "../Insufficient_Arg_Support/data-splitting.tsv"
...
def map_labels_to_tokens(token_list, label_list):
    h = 0
    j = 0
    while h < len(tokens_list) and j < len(original_range_labels):

        if tokens_list[h] == original_range_words[j].lower():
            label_sequence.append(original_range_labels[j])
            j += 1
            h += 1
        else:
            idx_range = len(tokenizer.tokenize(original_range_words[j]))
            delta = 0
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    #sentence = " ".join(example.token_list)
    #tokens_a = tokenizer.tokenize(sentence)
    # todo - tokenize words and lowercase- how to deal with masking?
    # todo unten entfernen ist nur zur Sicherheit - 28.12
    tokens_list = [word.lower() for word in example.token_list]
    label_sequence = example.label_list
    assert len(label_sequence) == len(tokens_list)

    tokens_list = tokenizer.tokenize(" ".join(example.token_list))
    label_sequence = []

    original_range_words = example.token_list
    original_range_labels = example.label_list
    if len(tokens_list) > len(original_range_labels):
        # map_labels_to_tokens(tokens_list, original_range_labels)
        h = 0
        j = 0
        while h < len(tokens_list) and j < len(original_range_labels):
        ...
        label_sequence = original_range_labels

    assert len(label_sequence) == len(tokens_list)
    # TODO should not be too long - but just to make sure the cut is made here (again)
    if len(tokens_list) > max_seq_length - 2:
        tokens_list = tokens_list[0:(max_seq_length - 2)]
    ...
    input_ids = tokenizer.convert_tokens_to_ids(tokens) # TODO zero PAdding of seq. Length
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["label_ids"] = create_int_feature(feature.label_id) # todo here do [] brackets
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    #
    # logits = project_layer(output_layer, num_labels, seq_length, scope='project')
    ...
    # print('embedding_size:%d' % hidden_size)
    ...
    with tf.variable_scope("loss"):
        ...
        # logits2 = tf.tensordot(output_layer, output_weights, axes=1)
        logits = tf.matmul(output_layer, output_weights, transpose_b=True)
          # logits2 = tf.tensordot(output_layer, output_weights, axis =1, transpose_b=True)
        ...
        #loss = tf.reduce_mean(per_example_loss)
        """
        Above works but the loss might not be calculated correctly
        Further testing on the correct evaluation 
        """
        # Approach 1 - using input ids
        # boolean tensor to check if ids > 0
        # tensor1 with 1, and tensor2 with 0
        # depending on the boolean geturn 1 o 0 =>> padding mask
        # paddings mark - alter type to float >> weights_sequence

        # non_padding_tokens = tf.greater(input_ids, 0)  # True if greater than 0 -> it is no padding example
        # zeros = tf.zeros([batch_size, seq_length], dtype=tf.float32)
        # ones = tf.fill([batch_size, seq_length], 1.0)
        # padding_mask = tf.where(non_padding_tokens, ones, zeros)
        # weights_sequence = tf.cast(padding_mask, dtype=tf.float32)
        # one_hot_labels = tf.one_hot(labels, depth=num_labels, dtype=tf.float32)
        # # 1-hot musst have same size as logits
        # loss_test = tf.losses.softmax_cross_entropy(
        ...
        # )
        # Approach 2 - using input mask >> 1 for real tokens, padding tokens are 0
        # boolean tensor to check if ids > 0
        # tensor1 with 1, and tensor2 with 0
        # depending on the boolean geturn 1 o 0 =>> padding mask
        # paddings mask - alter type to float >> weights_sequence
        print("Input_MASK\n", input_mask)
        ...
        return (loss, per_example_loss, logits, probabilities)
         #  TODO this need to be added somehow for padding (and to deal with the word pieces)?
        # weights_sequence = tf.zeros([batch_size, seq_length])  # batch x height x width x color
        # loss = tf.losses.softmax_cross_entropy(
        #     onehot_labels=None,
        #     logits=logits,
        #     weights=1.0,
    ...
    # print(len(unstacked_logits))
    #
    # return unstacked_logits
    # from squad dont think i need this
        ...
        d = tf.data.Dataset.from_tensor_slices({
            ...
            "segment_ids":
                tf.constant(
                    ...
                    dtype=tf.int32),
            "label_ids":
                tf.constant(all_label_ids, shape=[num_examples, seq_length], dtype=tf.int32), # TODO changed this
        })
