...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("second_stage_output"):
        if is_training:
            # I.e., 0.1 dropout
...
if __name__ == "__main__":
    ...
    BERT_BASE_DIR = ""  # TODO PATH
    BERT_STILT_DIR = ""  # TODO PATH
    BERT_finetuning_output_dir = ""  # TODO PATH

    FLAGS.task_name = "MNLI"
    FLAGS.do_train = False
    FLAGS.do_eval = True
    FLAGS.do_predict = False
    FLAGS.data_dir = ""  # TODO PATH
    FLAGS.vocab_file = BERT_BASE_DIR + "/vocab.txt"
    FLAGS.bert_config_file = BERT_BASE_DIR + "/bert_config.json"
    FLAGS.init_checkpoint = BERT_STILT_DIR
    FLAGS.max_seq_length = 128
    FLAGS.train_batch_size = "[32]"
    FLAGS.learning_rate = "[2e-5]"
    FLAGS.num_train_epochs = "[2]"
    FLAGS.output_dir = BERT_finetuning_output_dir + "/MNLI"

    # processor = MnliProcessor()
    # train = processor.get_train_examples(data_dir=FLAGS.data_dir)
    # dev = processor.get_dev_examples(data_dir=FLAGS.data_dir)
    # labels=[]
    # for ele in train:
