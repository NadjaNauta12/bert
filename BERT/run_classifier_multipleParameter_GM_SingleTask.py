...
def _convert_single_example_ACI(ex_index, example, label_list, max_seq_length,
    ...
    # should not be too long - but just to make sure the cut is made here (again)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("second_stage_output"): #  changed this for single task
        if is_training:
            # I.e., 0.1 dropout
...
def _create_model_ACI(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, num_labels, use_one_hot_embeddings):
    ...
    final_hidden_shape = modeling.get_shape_list(output_layer, expected_rank=3)  # embedding size
    batch_size = final_hidden_shape[0]
    seq_length = final_hidden_shape[1]
    hidden_size = final_hidden_shape[2]
    print("Sizes:", batch_size, "<>", seq_length, "<>", hidden_size)
    ...
    with tf.variable_scope("second_stage_output"):  # changed this for single task
        ...
        reshape_output = tf.reshape(output_layer, [-1, hidden_size])
        output_layer = reshape_output  # shape [batch_size*seq_length, hidden]

        # unnormalized output of last layer
        logits = tf.matmul(output_layer, output_weights, transpose_b=True)
        logits = tf.nn.bias_add(logits, output_bias)  # shape [batch_size*seq_length, num_labels]

        # convert to probabilities # axis -1 = last dim of tensor -> contains labels
...
def set_experimental_setting_ARTask(setting):
    # possible settings:  # "UGIP" #"GM" # "UGIP", "GM_UGIP", "UGIP_GM"
