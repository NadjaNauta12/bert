...
# for each bucket select the best model according to the loss score


def main():
    path = get_path_to_OS() + "/evaluation/BERTonSTILTs_Finetuning/MNLI_DEV_Scores_Consolidated.txt"
    AZI_bucket = []
    AQ_bucket = []
    AR_GM_bucket = []
    AR_GM_UGIP_bucket = []
    ...
    assert (len(AQ_bucket) == 6)
    assert (len(AZI_bucket) == 6)
    assert (len(ACI_Stab_bucket) == 6)
    assert (len(ACI_Lauscher_bucket) == 6)
    assert (len(ISA_bucket) == 6)
    assert (len(AR_GM_bucket) == 6)
    assert (len(AR_GM_UGIP_bucket) == 6)
...
def _write_best_target_models_Dev_score(best_model_each):
    path = get_path_to_OS() + "/evaluation/BERTonSTILTs_Finetuning/Best_DEV_Score_MNLI.txt"
