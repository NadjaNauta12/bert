...
def main():
    ...
    if FineTuning:
        for i, name in enumerate(task_names):
            ...
            _write_best_model_overview(get_path_to_OS() + "/evaluation/BERTonSTILTs_Finetuning/Consolidated_Modelevaluation.txt",
    ...
    if MNLI_Validation:
        ...
        for i, name in enumerate(task_names):
            ...
            models = get_all_models_for_task(path, name)  # tuple of 4
            all_target_models.append(models)

        print( len(models))# == 4  # tuple of 4
        print( len(combinations)) #== 6  # 6 other tasks targeted -string
        print( len(all_target_models))# == 6  # 6 other tasks targeted -models

        assert len(models) == 4  # tuple of 4
        assert len(combinations) == 7  # 6 other tasks targeted -string
        assert len(all_target_models) == 7  # 6 other tasks targeted -models

        _write_best_model_overview(get_path_to_OS() + "/evaluation/BERTonSTILTs_Finetuning_validation/MNLI/Consolidated_MNLI_TargetModelEvaluation.txt",
                                   all_target_models, combinations)
    ...
    if Validation:
        ...
        for i, name in enumerate(task_names):
            ...
            print("LENGTH", len(combinations), len(all_target_models))
            assert len(models) == 4  # tuple of 4
            assert len(combinations) == 3 or len(combinations) == 2  # 3 other tasks targeted -string
            assert len(all_target_models) == 3 or len(all_target_models) == 2  # 3 other tasks targeted -models
