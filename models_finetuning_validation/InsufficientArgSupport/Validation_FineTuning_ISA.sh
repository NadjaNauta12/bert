...
if [ "$OS" = "Windows_NT" ] ; then
   echo "Windows_NT"
    ...
    export BERT_finetuning_output_dir_VALIDATION="/models_finetuning_validation/InsufficientArgSupport"
    export BERT_onSTILTs="/models_onSTILTs/models/InsufficientArgSupport_32_3e-05_3/model.ckpt-69"
    # # # # # # # # # #
    export VOCAB_DIR=$BERT_BASE_DIR/vocab.txt
    export BERT_CONFIG=$BERT_BASE_DIR/bert_config.json

    echo ">>>>> TRAINING <<<<<"
#    for task_name in   "ArgZoningI" "ACI_Lauscher"  ; do
#        echo $task_name
#        case $task_name in
